...
/*   Updated: 2023/04/24 23:11:34 by srapin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_io(t_param *param)
{
	if (param->infile && access(param->infile, F_OK | R_OK) == -1)
		perror("acces infile");
	if (access(param->outfile, F_OK) != -1 && access(param->outfile, W_OK)
		== -1)
		perror("acces outfile");
	if (access(param->outfile, F_OK) == -1)
		safe_close(&(int){open(param->outfile, O_WRONLY | O_CREAT, 0666)});
}
